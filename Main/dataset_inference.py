...
class InferenceRecipe:
    ...
    def generate(self, cfg: DictConfig):
        ...
        decoded_output = self._tokenizer.decode(generated_tokens[0])
        ...
        # return the decoded output,time for inference, tokens per second, bandwidth achieved and memory used

        return decoded_output, t, tokens_sec, model_size * tokens_sec / 1e9, torch.cuda.max_memory_allocated() / 1e9
...
def main(cfg: DictConfig) -> None:
    ...
    # create a new dataframe called output data
    output_data = pd.DataFrame(columns=['response','expected_response', 'time', 'tokens/sec', 'bandwidth', 'memory'])
    ...
    for i in tqdm(range(len(df))):
        ...
        prompt = df.iloc[i]
        cfg.prompt = {"system": None,"user": prompt['text']}

        decoded_output, t, tokens_sec, bandwidth, memory = recipe.generate(cfg=cfg)

        # finding the place indices where "-> ?" is present in the decoded output
        idx = decoded_output.find("-> ?")

        # if the decoded output contains "-> ?", then remove the text before it
        if idx != -1:
            decoded_output = decoded_output[idx+4:]
        
        # append the output to the output_data dataframe use other than .append method
        output_data.loc[i] = [decoded_output,prompt['label'], t, tokens_sec, bandwidth, memory]

    output_data.to_csv(cfg.output_dataset_path, index=False)
